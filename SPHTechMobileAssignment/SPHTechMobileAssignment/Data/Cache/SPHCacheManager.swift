...
public class SPHCacheManager {
    ...
    private var cacheQueue = DispatchQueue.init(label: "com.sph.cacheQueue")
    
    public static var sharedInstance = SPHCacheManager.init(cacheName: Bundle.main.infoDictionary?["SPHTechMobileAssignment"] as? String ?? "MyAppCache")
    ...
    private init(cacheName: String) {
        if let cacheDirectory = NSSearchPathForDirectoriesInDomains(FileManager.SearchPathDirectory.cachesDirectory, FileManager.SearchPathDomainMask.userDomainMask, true).last {
            let dir = cacheDirectory + "/SHPCache/" + cacheName
